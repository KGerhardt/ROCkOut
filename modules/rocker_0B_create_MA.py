...
class MA_manager:
	...
	def access_files(self):
		...
		self.target_paths = self.roc_proj.targets_nt
	...
	def create_ma(self):
		self.ma = os.path.normpath(self.in_dir + "/shared_files/multiple_alignment/target_seq_MA.fasta")
	...
	def create_tree(self):
		...
		self.log_file = os.path.normpath(self.in_dir + "/shared_files/multiple_alignment/fasttree_log.txt")
		...
		fasttree_command = ["fasttree", "-nt", "-gtr", "-log", self.log_file, "-out", self.tree, self.ma]
		subprocess.run(fasttree_command)


def make_ma(parser, opts):
	project_directory = opts.dir
	if project_directory is None:
		print("You need to specify a ROCkOut project directory. Quitting.")
		print(parser.print_help())
		quit()
	manager = MA_manager(project_directory)
